...
const MobileAccessoriesCategory = () => {
  const [items, setItems] = useState<MobileAccessoryProduct[]>(mobileAccessoriesProducts);
  ...
  useEffect(() => {
    api.fetchProducts({ category: "mobile-accessories", limit: 200 })
      .then((data) => {
  ...
  const filteredProducts = useMemo(() => {
    let result: MobileAccessoryProduct[] = [...items];
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container-main py-6">
        ...
        <div className="flex gap-8">
          <AccessoryFilterSidebar filters={filters} onChange={handleFilterChange} onReset={handleReset} brands={brands} />
          <AnimatePresence>
            {showMobileFilters && (
              <AccessoryFilterSidebar filters={filters} onChange={handleFilterChange} onReset={handleReset} brands={brands} isMobile onClose={() => setShowMobileFilters(false)} />
            )}
          </AnimatePresence>
